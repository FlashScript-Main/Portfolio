    ...
    return (
        <GlobalMotionMain className={`overflow-x-hidden | selection:text-nftCustom-cta selection:bg-nftCustom-text |  | ${isFarsi && `${iranSans}`}`}>
            <section className={`h-screen w-full |  | grid place-content-center | border-2 border-indigo-500`}>
                <div className={`w-[80vw] h-[80vh] overflow-hidden |  | grid grid-cols-4 gap-4 | border-2 border-green-600`}>
                    <div className={`w-full h-full | bg-green-500 | row-span-2  | `}></div>
                    <div className={`w-full h-full | bg-red-500 |  | `}></div>
                    <div className={`w-full h-full | bg-indigo-500 | col-span-2  | `}></div>
                    <div className={`w-full h-full | bg-rose-600 |  | `}></div>
                    <div className={`w-full h-full | bg-rose-100 |  | rounded-br-[20px]`}></div>
                    <div className={`w-full h-full | bg-yellow-500 | col-start-2 row-start-2 col-end-4 row-end-4  max-md:col-start-1 max-md:col-end-3 max-md:row-start-1 max-md:row-end-1 | rounded-b-[20px]`}></div>
                    <div className={`w-full h-full | bg-black | row-span-2  | rounded-bl-[20px]`}></div>
                    <MotionDiv 
                        initial={{ y: "20%", x: `${isWide ? "20%" : "-20%"}`, opacity: 0 }}
                        whileInView={{ y: "0%", x: "0%", opacity: 1 }}
                        viewport={{ once: true, margin: "-50px" }}
                        transition={{ delay: 0.25, duration: 0.5, ease: "easeInOut" }}
                        className={`w-full h-full | bg-purple-800  | col-span-2 | `}
                    ></MotionDiv>
                    <div className={`w-full h-full | bg-rose-900 |  | `}></div>
