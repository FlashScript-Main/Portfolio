...
const Home = ({ params: { locale } }: MainPagePropsType) => {
    ...
    return (
        <GlobalMotionMain className={`overflow-x-hidden | selection:text-nftCustom-cta selection:bg-nftCustom-text |  | ${isFarsi && `${iranSans}`}`}>
            <Navbar />
            <section className={`h-screen w-full |  | grid place-content-center | border-2 border-indigo-500`}>
                {/* w-[80vw] h-[80vh] | grid-rows-9 md:grid-rows-5 lg:grid-rows-4 */}
                <DivToScroll className={`w-full h-[80vh] mt-20 md:mt-24 sm:w-[600px] md:w-[730px] lg:w-[950px] xl:w-[1200px] 2xl:w-[1500px] max-sm:px-4 mx-auto overflow-hidden |  | grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  xl:grid-cols-4 gap-4 | border-2 border-green-600`}>
                    <div className={`w-full h-full | bg-green-500 | lg:max-xl:row-start-2 lg:max-xl:col-start-3 xl:row-span-2 | `}>Skills</div>
                    <div className={`w-full h-full | bg-red-500 | lg:max-xl:row-start-4 lg:max-xl:col-start-1 | `}>{"I'm 22 from Iran"}</div>
                    ...
                    <div className={`w-full h-full | bg-rose-100 | max-md:row-start-5 lg:max-xl:row-start-4 lg:max-xl:col-start-3 | rounded-br-[20px]`}>26+ Projects</div>
                    <div className={`w-full h-full | bg-yellow-500 | max-lg:row-start-1 md:col-span-2 lg:max-xl:row-span-2 lg:max-xl:row-start-2 xl:col-start-2 xl:row-start-2 xl:col-end-4 xl:row-end-4 | rounded-b-[20px]`}>FlashScript</div>
                    <div className={`w-full h-full | bg-black text-white | max-md:row-start-3 md:max-lg:row-start-2 md:max-lg:col-start-2 lg:max-xl:row-start-3 lg:max-xl:col-start-3 xl:row-span-2 | rounded-bl-[20px]`}>Projects</div>
                    <MotionDiv 
                        initial={{ y: "20%", x: `${isWide ? "20%" : "-20%"}`, opacity: 0 }}
                        whileInView={{ y: "0%", x: "0%", opacity: 1 }}
                        viewport={{ once: true, margin: "-50px" }}
                        transition={{ delay: 0.25, duration: 0.5, ease: "easeInOut" }}
                        className={`w-full h-full | bg-purple-800  | xl:col-span-2 | `}
                    >Contact Me: Github | LinkedIn | X</MotionDiv>
                    <div className={`w-full h-full | bg-rose-900 | lg:max-xl:row-start-1 lg:max-xl:col-start-1 | `}>Freelancer | Currently Available To Work</div>
                </DivToScroll>
